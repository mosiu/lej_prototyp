...
hd44780_pulse_e(uint8_t readback) __attribute__((always_inline));

static inline uint8_t
hd44780_pulse_e(uint8_t readback)
{
  uint8_t x;
 ...
 */
void
hd44780_wait_ready(uint8_t longwait)
{
#if USE_BUSY_BIT
  while (hd44780_incmd() & HD44780_BUSYFLAG) ;	//this loop will continue until the busy flag is cleared.
												//So this library turned out to use polling.
#else
  if (longwait)
    _delay_ms(1.52);
  else
...
hd44780_init(void)
{
  ...
  hd44780_wait_ready(0);
  hd44780_outcmd(HD44780_DISPCTL(0, 0, 1));
  hd44780_wait_ready(0);
}
